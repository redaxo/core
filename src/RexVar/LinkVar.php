...
final readonly class LinkVar
{
    ...
    public static function getWidget(int|string $id, string $name, ?int $value, ?int $category = null): string
    {
        ...
        $e = [];
        $e['field'] = '<input class="form-control" type="text" name="REX_LINK_NAME[' . $id . ']" value="' . escape($artName) . '" id="REX_LINK_' . $id . '_NAME" readonly="readonly" /><input type="hidden" name="' . $name . '" id="REX_LINK_' . $id . '" value="' . escape((string) $value) . '" />';
        $e['functionButtons'] = '
