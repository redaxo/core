...
abstract class rex_error_handler
{
    ...
    private static function renderWhoops($exception)
    {
        ...
        if ($markdownReport) {
            ...
            $reportBugLink = '<a class="rex-report-bug" href="https://github.com/redaxo/core/issues/new?labels=' . rex_escape($bugLabel, 'url') . '&title=' . rex_escape($bugTitle, 'url') . '&body=' . rex_escape($bugBodyCompressed, 'url') . '" rel="noopener noreferrer" target="_blank">Report a REDAXO bug</a>';
        }
