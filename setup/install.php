...
Table::get('rex_cronjob')
    ...
    ->ensureColumn(Column::varchar('type', 255))
    ->ensureColumn(Column::text('parameters', nullable: true))
    // JSON with five interval parts. Selecting every single option the form offers comes to 277
    // characters, so this leaves room for finer-grained parts without another schema change.
    ->ensureColumn(Column::varchar('interval', 500))
...
Table::get('rex_media')
    ->ensurePrimaryIdColumn()
    ->ensureForeignIdColumn('category_id', 'rex_media_category', nullable: true)
    ->ensureColumn(Column::varchar('filetype', 255, nullable: true))
    ->ensureColumn(Column::varchar('filename', 255))
    ->ensureColumn(Column::varchar('originalname', 255))
...
Table::get('rex_user')
    ...
    ->ensureGlobalColumns()
    ->ensureColumn(Column::datetime('password_changed', nullable: true))
    ->ensureColumn(Column::text('previous_passwords', nullable: true))
